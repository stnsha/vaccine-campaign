...
$row = $num > 0 ? $result -> fetch_assoc() : null;
$id = $row ? stripslashes($row['id'] ?? '') : '';
$phone = $row ? stripslashes($row['phone'] ?? '') : '';
$phone_parts = explode("@",$phone);
$customer_name = $row ? stripslashes($row['customer_name'] ?? '') : '';
$phone_child = isset($phone_parts[1]) ? $phone_parts[1] : '';

if ($result)
{
echo "<img src='../common/img/tick.png' title='Ok'>|$phone_parts[0]|$ic|$customer_name|$phone_child";
}
}
